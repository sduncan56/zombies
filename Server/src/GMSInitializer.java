...
public class GMSInitializer implements Plugin, PluginLifeCycle {
  ...
  public void init(EsObjectRO ignored) {
      ...
      String extensionName = "ZombieExtension";


      // invoke the initialization method for each of your games
      initGame1(extensionName);

  }

  private void initGame1(String extensionName) {
      ...
      gamePlugin.setHandle("ZombiePlugin");//Name by which the plugin can be addressed when instantiated in the room
      ...
      gamePlugin.setName("ZombiePlugin");//Name of the plugin in the Extension.xml file

      // Create the room configuration
      RoomConfiguration roomConfig = new RoomConfiguration();
      roomConfig.setCapacity(-1);
      roomConfig.setDescription("Zombie game");
      ...
      getApi().registerGameConfiguration("ZombiePlugin", gameRoomConfig);

      System.out.println("Zombie plugin initialized.");
  }
